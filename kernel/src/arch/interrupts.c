...
int do_execve(const char* path, const char* argv[], const char* envp[]) {
    ...
    if (fd < 0) return -1; // 文件不存在

    // 2. 获取文件大小 (为了分配缓冲区读取整个文件)
    // 这里偷懒直接读，实际上应该分块读或 mmap
    struct { uint64_t size; } st; // 简易 stat 结构
    ...
    if (!elf_buf) { ramfs_close(fd); return -1; }
    
    int read_bytes = ramfs_read(fd, elf_buf, st.size);
    ramfs_close(fd);

    if (read_bytes != st.size) { kfree(elf_buf); return -1; }
...
void syscall_handler(registers_t *regs) {
    ...
    if (current_proc) current_proc->trap_frame = (trap_frame_t*)regs;
    ...
    switch (syscall_num) {
        ...
        case 1: // SYS_WRITE (fd, buf, count)
            if (arg1 == 1 || arg1 == 2) { // STDOUT/STDERR
                char* s = (char*)arg2;
                int len = (int)arg3;
                for(int i=0; i<len; i++) kprint_char(s[i]);
                ret = len;
            } else {
        ...
        case 79: // SYS_GETCWD (buf, size)
        {
            char* buf = (char*)arg1;
            // 极简：永远在根目录
            if(arg2 > 1) { strcpy(buf, "/"); ret = (uint64_t)buf; }
            else ret = 0; 
            break;
        }
        ...
        case 217: // SYS_GETDENTS64 (fd, dirp, count)
            ret = ramfs_getdents64((int)arg1, (void*)arg2, (int)arg3);
            break;
        ...
        case 110: // SYS_GETPPID
            if (current_proc->parent) ret = current_proc->parent->pid;
            else ret = 0;
            break;
